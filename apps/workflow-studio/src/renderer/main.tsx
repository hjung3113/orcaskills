...
function Studio() {
  ...
  const selectedNodeOverride = selectedNode ? document.workflow?.nodeProfileOverrides?.[selectedNode.id] : undefined;
  ...
  function selectNodeProfile(profileId: string) {
    const workflow = document.workflow;
    if (!workflow || !selectedNode) return;
    const overrides = { ...workflow.nodeProfileOverrides };
    if (profileId) overrides[selectedNode.id] = { profileId };
    else delete overrides[selectedNode.id];
    commit({ ...workflow, ...(Object.keys(overrides).length ? { nodeProfileOverrides: overrides } : { nodeProfileOverrides: undefined }) });
  }

  function applyPresetToSelectedNode(presetId: string) {
    if (!presetId || !document.workflow || !selectedNode) return;
    const applied = applyPortablePreset(portableConfiguration, presetId);
    const overrides = { ...document.workflow.nodeProfileOverrides, [selectedNode.id]: { profileId: applied.preset.profileId, ...(applied.preset.modelPolicy ? { modelPolicy: applied.preset.modelPolicy } : {}) } };
    commit({ ...document.workflow, nodes: document.workflow.nodes.map((node) => node.id === selectedNode.id ? { ...node, roleId: applied.preset.roleId } : node), nodeProfileOverrides: overrides });
    setPortableConfiguration(applied.configuration);
